...
- `sdkconfig.defaults*` changes are detected automatically: those files are not in ninja's reconfigure trigger list (an upstream blind spot), so `build_project` merges the defaults assignments into `sdkconfig` in place before every build (values overridden, missing lines appended, no file deleted; symbols not mentioned in the defaults are never touched). Defaults edits take effect without manually deleting `sdkconfig`.
