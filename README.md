...
| `monitor_open` / `monitor_read` / `monitor_send` / `monitor_close` | Persistent interactive serial session; `monitor_read` starts at `Returned from app_main()` (everything before folded into a count), collapses adjacent duplicates as `<first line>  *N`, offers `full=True` for a cursor-independent dump of everything retained, `timestamp=True` to keep the ms prefix, and writes every line to `<project>/.esp_monitor_full.log`; `monitor_close(session_id)` (or a bare port name) releases exactly that session / port; a session auto-releases after `idle_release` seconds (default 30) without read/write |
