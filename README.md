...
- **Builds through `idf.py build`** — the exact same path as a manual build, so hand-edited `sdkconfig` values are picked up through the official ninja RERUN_CMAKE flow instead of being silently reverted by a kconfgen rewrite. `sdkconfig.defaults*` entries (an upstream blind spot — ninja never notices them, and kconfgen lets stale `sdkconfig` entries win) are merged into `sdkconfig` in place before every build: values overridden, missing lines appended, nothing else touched, no file deleted; backup files such as `.bak`/`.old` are ignored. Remove an option from the defaults files to stop pinning it.
